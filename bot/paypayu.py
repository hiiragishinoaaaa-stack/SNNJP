...
async def login(phoneNumber: str, password: str, uuid: str):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post("https://www.paypay.ne.jp/app/v1/oauth/token", headers=headers, json=payload, proxy=PROXY_URL or None) as response:
...
async def check_link(cd):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(f"https://www.paypay.ne.jp/app/v2/p2p-api/getP2PLinkInfo?verificationCode={cd}", headers=headers, proxy=PROXY_URL or None) as response:
...
async def link_rev(cd: str, phoneNumber: str, password: str, uuid: str, link_password: str = None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(f"https://www.paypay.ne.jp/app/v2/p2p-api/getP2PLinkInfo?verificationCode={cd}", headers=base_headers, proxy=PROXY_URL or None) as response:
                response.raise_for_status()
                link_info = await response.json()
        ...
        async with session.post("https://www.paypay.ne.jp/app/v1/oauth/token", headers=login_headers, json=login_payload, proxy=PROXY_URL or None) as response:
            login_response = await response.json()
            try:
        ...
        try:
            async with session.post("https://www.paypay.ne.jp/app/v2/p2p-api/acceptP2PSendMoneyLink", json=receive_payload, headers=base_headers, proxy=PROXY_URL or None) as response:
                response.raise_for_status()
