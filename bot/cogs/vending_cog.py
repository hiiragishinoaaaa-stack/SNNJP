...
import paypayu
from paypay_cog import load_paypay_data, PAYPAY_DATA_FILE, PayPayModal
...
# BOTオーナーのユーザーID (.envの ADMIN_USER_ID に設定)
ADMIN_USER_ID = int(os.getenv("ADMIN_USER_ID", "0"))

# APIキー (.envの SMM_API_KEY に設定)
SMM_API_KEY = os.getenv("SMM_API_KEY", "")

# 全サーバーの実績を送信する共通ログチャンネルID (.envの GLOBAL_LOG_CHANNEL_ID に設定)
GLOBAL_LOG_CHANNEL_ID = int(os.getenv("GLOBAL_LOG_CHANNEL_ID", "0"))

# オーナー専用コマンドの名前(変えなくてもOK)
ADMIN_LABEL = "《オーナー専用》"

# embedのフロッパー(自分の名前に変更)
BOT_FUROPA = os.getenv("BOT_FOOTER", "BOT製作者:@ehuru_078")
...
class PayPayInputModal(ui.Modal, title="最終決済"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        owner = load_paypay_data().get(str(ADMIN_USER_ID))
        if not owner:
            return await interaction.followup.send("❌ 決済システムエラー: 管理者のPayPay情報が未設定です。", ephemeral=True)
        ...
        success = await paypayu.link_rev(self.paylink.value, owner['phone'], owner['password'], owner['uuid'])
        ...
        if success is True:
            ...
            if "order" in res:
                ...
                if global_chan:
                    try:
                        await global_chan.send(embed=l_emb)
                    except:
                        pass
            ...
            async def on_submit(self, inter: discord.Interaction):
                ...
                try:
                    ...
                    if "status" in res:
                        ...
                        await inter.followup.send("注文が見つかりませんでした。IDを確認してください。", ephemeral=True)
                except Exception as e:
...
class VendingCog(commands.Cog):
    ...
    async def paypay_login(self, interaction: discord.Interaction, phone: str, password: str):
        ...
        if result.get("response_type") == "ErrorResponse":
            ...
            return
        modal = PayPayModal(phone, password, set_uuid, result.get("otp_reference_id"), result.get("otp_prefix"))
        await interaction.response.send_modal(modal)

    @app_commands.command(name="フォロ爆paypayログアウト", description=f"{ADMIN_LABEL}PayPay情報を削除します")
    @is_admin()
    async def paypay_logout(self, interaction: discord.Interaction):
        data = load_paypay_data()
        if str(ADMIN_USER_ID) in data:
            del data[str(ADMIN_USER_ID)]
    ...
    async def log_set(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        gid = str(interaction.guild.id)
        if gid not in settings["guilds"]:
            settings["guilds"][gid] = {"balance": 0, "total_sales": 0}
